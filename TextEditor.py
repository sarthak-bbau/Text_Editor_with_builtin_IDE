...
main_application.title('text editor cum IDE')
...
# Create a new menu for speech-to-text functionality
speech_menu = tk.Menu(main_menu, tearoff=False)
main_menu.add_cascade(label='Speech', menu=speech_menu)

# Add speech-to-text command to the speech menu
speech_menu.add_command(label='Speech to Text', command=speech_to_text)
...
def check_spelling():
    ...
    text_editor.delete(1.0, tk.END)
    text_editor.insert(tk.INSERT, corrected_content)



# Spelling check functionality
check_spelling_btn = ttk.Button(tool_bar, text='Check Spelling', command=check_spelling)
check_spelling_btn.grid(row=0, column=9, padx=5)
...
def call_ide():
    subprocess.run(['python', 'Ide.py'])

# IDE button
ide_button = ttk.Button(tool_bar, text='IDE', command=call_ide)
ide_button.grid(row=0, column=10, padx=5)
